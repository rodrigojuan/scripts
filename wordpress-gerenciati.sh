...
DATABASE="CREATE DATABASE gerenciati;"
...
GRANTALL="GRANT ALL PRIVILEGES ON gerenciati.* TO 'costabrava';"
...
echo -e "Após a instalação do Wordpress acessar a URL: http://`hostname -I`/gerenciati/\n"
...
echo -e "Copiando os arquivos de configuração do Wordpress, aguarde..."
	...
	mv -v wordpress/ /var/www/html/gerenciati &>> $LOG
	cp -v conf/htaccess /var/www/html/gerenciati/.htaccess &>> $LOG
	cp -v conf/wp-config.php /var/www/html/gerenciati/ &>> $LOG
...
echo -e "Alterando as permissões dos arquivos e diretórios do Wordpress, aguarde..."
	...
	chmod -Rfv 755 /var/www/html/gerenciati/ &>> $LOG
	chown -Rfv www-data.www-data /var/www/html/gerenciati/ &>> $LOG
...
echo -e "Editando o arquivo de configuração da Base de Dados do Wordpress, aguarde..."
	echo -e "Pressione <Enter> para editar o arquivo: wp-config.php"
		...
		read
		sleep 3
	vim /var/www/html/gerenciati/wp-config.php
...
echo -e "Editando o arquivo de configuração do htaccess do Wordpress, aguarde..."
	echo -e "Pressione <Enter> para editar o arquivo: .htaccess"
		...
		read
		sleep 3
	vim /var/www/html/gerenciati/.htaccess
echo -e "Arquivo editado com sucesso!!!, continuando com o script..."
sleep 5
echo
#
